...
my @genes = qw/KRAS TP53 CDKN2A SMAD4 MAP2K4 ARID1A RNF43 TGFBR2 KDM6A/;
my %geneHash;
for my $gene (@genes)
{
	$geneHash{$gene}++;
...
my %varMap = (
	...
	"stoploss" => "missense",
	"nonframeshift" => "missense",
	"stopgain" => "nonsense",
	"frameshift" => "nonsense",
	"deletion breakpoint" => "nonsense",
	"duplication breakpoint" => "nonsense",
	"inversion breakpoint" => "nonsense",
	"translocation breakpoint" => "nonsense",
	"splicing" => "splicing",
	"altered promoter" => "noncoding",
	"altered enhancer" => "noncoding",
);
...
while ($l = <FILE>)
{
	...
	else
	{
		...
		if (exists $geneHash{$row{gene}})
		{
			$data{$row{tumour}}{"$row{gene}:cn"} = $row{copy_number};
			if (($row{mutation_class} eq "somatic snv") or ($row{mutation_class} eq "somatic indel") or ($row{mutation_class} eq "somatic sv"))
			{
				if (exists $varMap{$row{mutation_type}})
				{
					$data{$row{tumour}}{"$row{gene}:$varMap{$row{mutation_type}}"}++;
				}
			}
		}
	}
...
#		push(@yBottomCopy, (($numSamples - $i) - 1) - $yPad*($i/($numSamples-1)) + $yPad);
		...
		push(@yTopFirst, 1);
		push(@yBottomFirst, 1 - $firstHitSize);
		push(@yTopSecond, 1 - $firstHitSize);
		push(@yBottomSecond, 1 - $firstHitSize - $secondHitSize);
		push(@yTopCopy, 1 - $yPad - $firstHitSize - $secondHitSize);
		push(@yBottomCopy, 0);

		push(@xMarks, $j + 0.5);
		push(@yMarksFirst, 1 - ($firstHitSize / 2));
		push(@yMarksSecond, 1 - $firstHitSize - ($secondHitSize/2));
...
for $type (qw/snv_ca snv_cg snv_ct snv_ta snv_tc snv_tg/)
{
	for my $samp (@samples)
	{
		$data{$samp}{$type} = $data{$samp}{$type} / $data{$samp}{snv_count}
	}
}

for $type (qw/del_1_count del_4_count ins_1_count ins_4_count/)
{
	for my $samp (@samples)
	{
		$data{$samp}{$type} = $data{$samp}{$type} / $data{$samp}{indel_count}
	}
}

for $type (qw/sv_del_count sv_dup_count sv_inv_count sv_tra_count/)
{
	for my $samp (@samples)
	{
		$data{$samp}{$type} = $data{$samp}{$type} / $data{$samp}{sv_count}
	}
}
...
print $rfile "png(filename = \"$fileName-$plotType-1800_sample.png\", width = 1800, height = 150)\n";

print $rfile "par(cex=2)\n";

# onco
print $rfile "par(fig=c($oncoLeft,$oncoRight,0,1), mar=c($botMar, 8, $topMar,0) + 0.1)\n";
...
print $rfile "axis(2, at=($numSamples:1)-.5, labels=rev(donors[sort_order]), las=2, tick=FALSE, cex.axis=$axisCex,line=-0.5)\n";
...
print $rfile "barplot(matrix(rbind(snv_ca[sort_order],snv_cg[sort_order],snv_ct[sort_order],snv_ta[sort_order],snv_tc[sort_order],snv_tg[sort_order],zeros,zeros,zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($snvSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",ylim=c(0,1),horiz=TRUE,width=c(1 - $firstHitSize - $yPad,$firstHitSize),space=0, border=\"NA\")\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,zeros,zeros,snv_count[sort_order],zeros,zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1-$firstHitSize,$firstHitSize),add=T,space=c(0,0), border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SNVs\",side=3, line=1, cex=2.5)\n";
...
print $rfile "barplot(matrix(rbind(del_1_count[sort_order],del_4_count[sort_order],ins_1_count[sort_order],ins_4_count[sort_order],zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($indelSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize - $yPad, $firstHitSize),space=0, border=\"NA\", ylim=c(0,1))\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,indel_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"Indels\",side=3,line=1, cex=2.5)\n";


# sv count
print $rfile "par(fig=c($svLeft,$svRight,0,1), mar=c($botMar, 0, $topMar, 0) + 0.1, new=TRUE)\n";
my $svSubCol = "\"#F1932D\",\"#90C987\",\"#1965B0\",\"#882E72\"";
print $rfile "barplot(matrix(rbind(sv_del_count[sort_order],sv_dup_count[sort_order],sv_inv_count[sort_order],sv_tra_count[sort_order],zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($svSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize - $yPad,$firstHitSize),space=0, border=\"NA\", ylim=c(0,1))\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,sv_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SVs\",side=3,line=1, cex=2.5)\n";
...
print $rfile "mtext(\"CNV and SV across the Genome\",side=3,line=1,cex=2.5)\n";
...
print $rfile "png(filename = \"$fileName-$plotType-1800_sample_no_axis.png\", width = 1800, height = 50)\n";
...
# snv count
print $rfile "par(fig=c($snvLeft,$snvRight,0,1), mar=c($botMar, 0, $topMar, 0) + 0.1, new=TRUE)\n";
$snvSubCol = "\"#E8601C\",\"#F6C141\",\"#CAE0AB\",\"#4EB265\",\"#5289C7\",\"#B178A6\"";
print $rfile "barplot(matrix(rbind(snv_ca[sort_order],snv_cg[sort_order],snv_ct[sort_order],snv_ta[sort_order],snv_tc[sort_order],snv_tg[sort_order],zeros,zeros,zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($snvSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",ylim=c(0,1),horiz=TRUE,width=c(1 - $firstHitSize - $yPad,$firstHitSize),space=0, border=\"NA\")\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,zeros,zeros,snv_count[sort_order],zeros,zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1-$firstHitSize,$firstHitSize),add=T,space=c(0,0), border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SNVs\",side=3, line=1, cex=2.5)\n";
...
# indel count
print $rfile "par(fig=c($indelLeft,$indelRight,0,1), mar=c($botMar, 0, $topMar, 0) + 0.1, new=TRUE)\n";
$indelSubCol = "\"#DC050C\",\"#F7EE55\",\"#7BAFDE\",\"#D6C1DE\"";
print $rfile "barplot(matrix(rbind(del_1_count[sort_order],del_4_count[sort_order],ins_1_count[sort_order],ins_4_count[sort_order],zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($indelSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize - $yPad, $firstHitSize),space=0, border=\"NA\", ylim=c(0,1))\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,indel_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"Indels\",side=3,line=1, cex=2.5)\n";
...
# sv count
print $rfile "par(fig=c($svLeft,$svRight,0,1), mar=c($botMar, 0, $topMar, 0) + 0.1, new=TRUE)\n";
$svSubCol = "\"#F1932D\",\"#90C987\",\"#1965B0\",\"#882E72\"";
print $rfile "barplot(matrix(rbind(sv_del_count[sort_order],sv_dup_count[sort_order],sv_inv_count[sort_order],sv_tra_count[sort_order],zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c($svSubCol), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize - $yPad,$firstHitSize),space=0, border=\"NA\", ylim=c(0,1))\n";
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,sv_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SVs\",side=3,line=1, cex=2.5)\n";
...
print $rfile "png(filename = \"$fileName-$plotType-1000_no_name.png\", width = 1000, height = 130)\n";
...
# onco
print $rfile "par(fig=c($oncoLeft,$oncoRight,0,1), mar=c($botMar, 0, $topMar,0) + 0.1)\n";
print $rfile "plot(c(0,$numGenes), c(0,$numSamples), type=\"n\", xaxt=\"n\", yaxt=\"n\", xlab=\"\", ylab=\"\", bty=\"n\",xaxs=\"i\",yaxs=\"i\")\n";
print $rfile "rect(xLeft, yBottomFirst, xRight, yTopFirst, col=c(t(matrix(colFirst,$numSamples,$numGenes,byrow=TRUE)[rev(sort_order),1:$numGenes])), border=\"NA\")\n";
print $rfile "rect(xLeft, yBottomSecond, xRight, yTopSecond, col=c(t(matrix(colSecond,$numSamples,$numGenes,byrow=TRUE)[rev(sort_order),1:$numGenes])), border=\"NA\")\n";
print $rfile "rect(xLeft, yBottomCopy, xRight, yTopCopy, col=c(t(matrix(colCopy,$numSamples,$numGenes,byrow=TRUE)[rev(sort_order),1:$numGenes])), border=\"NA\")\n";
...
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,zeros,zeros,snv_count[sort_order],zeros,zeros,zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1-$firstHitSize,$firstHitSize),add=T,space=c(0,0), border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SNVs\",side=3, line=1, cex=1.5)\n";
...
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,indel_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"Indels\",side=3,line=1, cex=1.5)\n";
...
print $rfile "barplot(matrix(rbind(zeros,zeros,zeros,zeros,sv_count[sort_order],zeros,zeros,zeros),ncol=$numSamples*2), col=c(\"black\"), ylab=\"\", axes=FALSE, xlim=c(0,1),yaxs=\"i\",horiz=TRUE,width=c(1 - $firstHitSize,$firstHitSize),add=T,space=0, border=\"NA\")\n";
print $rfile "axis(3, at=c(0,0.5,1), labels=c(\"\",\"\",\"\"),line=0.2)\n";
print $rfile "mtext(\"SVs\",side=3,line=1, cex=1.5)\n";
...
print $rfile "mtext(\"CNV and SV across the Genome\",side=3,line=1,cex=1.5)\n";
print $rfile "axis(3, at=stripLabPos, labels=stripLab, las=2, tick=FALSE, cex.axis=$geneCex,line=-0.8)\n";
...
print $rfile "png(filename = \"$fileName-$plotType-1000_no_name-no_axis.png\", width = 1000, height = 50)\n";
...
print $rfile "png(filename = \"$fileName-$plotType-1000_slide.png\", width = 1024, height = 100)\n";
